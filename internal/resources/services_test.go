...
var _ = Describe("Services", func() {
	Context("Services()", func() {
		...
		It("places all services in the system namespace", func() {
			kn := testKubernaut()
			for _, svc := range Services(kn, testKnV2(kn), KagentiSidecarNone) {
		...
		It("SC-8: agent-tls always targets PortHTTPS where authbridge listens", func() {
			...
			for _, svc := range Services(kn, testKnV2(kn), KagentiSidecarAuthbridge) {
				if svc.Name == "apifrontend" {
					for _, p := range svc.Spec.Ports {
						if p.Name == AgentTLSPortName {
							Expect(p.TargetPort.IntValue()).To(Equal(int(PortHTTPS)),
	...
	Context("selectors", func() {
		It("use app labels that match known components", func() {
			kn := testKubernaut()
			all := append(Services(kn, testKnV2(kn), KagentiSidecarNone), MetricsServices(kn)...)
