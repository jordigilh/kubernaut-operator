...
- [#302](https://github.com/jordigilh/kubernaut-operator/issues/302) -- resolved by removing `InteractiveSpec.JWTProviders`/`AllowInsecureJWKS` outright rather than aligning their shape to Helm's. `kubernautAgentNetworkPolicy` only ever admits `AIAnalysis`/`APIFrontend` ingress to KA, and [kubernaut#1287](https://github.com/jordigilh/kubernaut/issues/1287) moved AF's own auth to KA onto an SA bearer token rather than JWT forwarding -- no caller this operator can admit would ever present a JWT to that field, and the CRD has no override to open an additional ingress source. See F6 addendum below
- [#298](https://github.com/jordigilh/kubernaut-operator/issues/298) -- closed the F2 `PrometheusSpec.{URL,TLSCaFile}` dead-field gap (zero non-test references anywhere in the codebase prior to this fix) and added a mirrored `AlertManagerSpec`. Also fixes two live-cluster-confirmed NetworkPolicy bugs uncovered while wiring the fix: EM's monitoring egress used an unscoped, wrong-port rule (`ipBlock: 0.0.0.0/0` on 9090/9093 instead of namespace-scoped 9091/9094), and AF/KA's shared egress rule never opened AlertManager's real post-DNAT container port (9095) alongside its documented Service port (9094). See F2 addendum below
...
| 1.7 | 2026-08-14 | Operator Team | F6 revisited for [#302](https://github.com/jordigilh/kubernaut-operator/issues/302): originally scoped as a field-shape alignment (singular `audience`/required `jwksURL`) between `InteractiveSpec.JWTProviders` (KA) and Helm's `kubernautAgent.interactive.jwtProviders[]`. Investigation found the field is unconditionally unreachable in this operator's own deployment topology, independent of any upstream shape: `kubernautAgentNetworkPolicy` (`internal/resources/networkpolicies.go`) admits only `AIAnalysis` and `APIFrontend` as ingress peers to KA's MCP port, and [kubernaut#1287](https://github.com/jordigilh/kubernaut/issues/1287) moved AF's own auth to KA onto a service-account bearer token (trusted intermediary) rather than forwarded end-user JWTs -- so no caller the operator can ever admit would present a JWT to KA's `interactive.jwtProviders` authenticator, and the CRD exposes no mechanism to open an additional ingress source. Rather than aligning the shape of dead configuration, `JWTProviders`/`AllowInsecureJWKS` are removed outright from `InteractiveSpec` in both `v1alpha1` and `v1alpha2` (the shared `JWTProviderSpec`/`ClaimMappingsSpec` types are retained -- `APIFrontendAuthSpec.JWTProviders` still uses them for AF's own live multi-provider auth). Consistent with the #288 precedent (v1.6, dead-field removal over shape-preserving rename). |
| 1.8 | 2026-08-15 | Operator Team | F2 revisited for [#298](https://github.com/jordigilh/kubernaut-operator/issues/298): `PrometheusSpec.{URL,TLSCaFile}` shipped in the v1alpha2 scaffold but were never actually wired into `internal/resources/configmaps.go` -- every consumer (EM, KA, AF severity-triage) hardcoded `OCPPrometheusURL` regardless of the field. Wired the override into all three, added a mirrored `AlertManagerSpec` (same shape/defaults) since EM/KA also need an overridable AlertManager endpoint, and implemented a three-tier NetworkPolicy egress resolver so the operator's own egress rule follows whichever URL is actually configured: (1) URL unset -> namespace-scoped to `openshift-monitoring`; (2) URL set and resolves to an in-cluster Service host (`<svc>.<ns>.svc[.cluster.local]`) -> namespace-scoped to the parsed namespace; (3) URL set and unresolvable (external) -> the operator omits its own egress rule for that destination, documented on `MonitoringSpec` that the platform operator must supply a supplemental `NetworkPolicy`. Live-cluster spike testing (kind+Calico, then OCP 4.18/OVN-Kubernetes) surfaced two pre-existing bugs fixed in the same change: EM's monitoring egress used an unscoped `ipBlock: 0.0.0.0/0` peer on the wrong ports (9090/9093 instead of 9091/9094) instead of `monitoringStackEgressRule`'s namespace-scoped one; and that shared rule (used by AF/KA) never opened AlertManager's real post-DNAT container port 9095 alongside its documented Service port 9094 -- OVN-Kubernetes evaluates egress ACLs after load-balancer DNAT (`apply-after-lb=true`, the same mechanism `dnsEgressRule` already documents for port 53->5353), so the Service-port-only rule never matched real AlertManager traffic on any OVN-Kubernetes cluster. Also tightened AF's rule to omit AlertManager entirely (AF's severityTriage config has no AlertManager client) as a least-privilege (SC-7) improvement. |
| 1.9 | 2026-08-26 | Operator Team | F3 revisited for [#444](https://github.com/jordigilh/kubernaut-operator/issues/444) (carved out of the #421/#422 field-coverage audit): `NetworkPoliciesSpec.ExternalRegistry` removed outright. Unlike every other `networkPolicies.*` egress override, there is no pod-level enforcement point this field could ever govern -- image pulls happen at the kubelet/CRI-O level for every pod, entirely outside any `NetworkPolicy`'s scope (which only governs traffic a running container's network namespace initiates), and this operator manages no in-cluster registry mirror or pull-through cache whose own egress a `NetworkPolicy` rule could scope instead. Consistent with the #288/#302/#302(F6) precedent of removing dead configuration outright rather than leaving it accepted-but-inert or shape-aligned to a target that doesn't exist. v1alpha2-only field, no v1alpha1 equivalent, no conversion-webhook impact. `networkPolicies.console.ingressNamespaces` (the other #422 carve-out) is resolved separately as intentionally-excluded-by-design under [#443](https://github.com/jordigilh/kubernaut-operator/issues/443) -- Console's ingress is user-driven browser/UI traffic, not a predictable set of in-cluster peers like every other component this ADR's `NetworkPolicyNamedIngressOverride` group covers, so the operator documents (`docs/security/credentials-and-tls.md`) how administrators can supply their own supplemental `NetworkPolicy` rather than the operator prescribing one. See `docs/tests/444/TEST_PLAN.md`. |
...
**Addendum (v1.8, #298)**: `PrometheusSpec.{URL,TLSCaFile}` above was implemented as scaffolded CRD fields at v1alpha2's original scaffold (D2/D3) but never actually consumed -- `effectivePrometheusURL(knV2)`/`effectiveAlertManagerURL(knV2)` (`internal/resources/common.go`) now wire the override into EM (`EffectivenessMonitorConfigMap`), KA (`kaToolsConfig`), and AF (`afSeverityTriageConfig`), falling back to `OCPPrometheusURL`/`OCPAlertManagerURL` when unset. Added `AlertManagerSpec` to `MonitoringSpec`, mirroring `PrometheusSpec`'s shape/defaults exactly (`Enabled *bool` default `true`, `URL string`, `TLSCaFile string`, `AlertManagerEnabled()`), since EM's `emExternalYAML.AlertManagerURL`/`AlertManagerEnabled` and KA's `tools.alertmanager` block have the same override need. `MonitoringSpec`'s own doc comment now documents the NetworkPolicy implication of an override (three-tier resolution, external hosts need a supplemental `NetworkPolicy`) so this stays discoverable from the CRD schema itself, not only this ADR.
...
type NetworkPoliciesSpec struct {
    ...
    // removed in v1.9 (#444) -- no pod-level enforcement point exists for
    // kubelet-level image pulls; see the addendum below.
    // +optional
...
**Addendum (v1.9, #444)**: `NetworkPoliciesSpec.ExternalRegistry` (the field list above) removed outright. Carved out of the #421/#422 field-coverage audit as unwireable: image pulls happen at the kubelet/CRI-O level for every pod, entirely outside any `NetworkPolicy`'s scope, and this operator manages no in-cluster registry mirror/pull-through cache whose egress a `NetworkPolicy` rule could scope instead. No pod-level enforcement point exists or is planned, so the field is removed rather than left accepted-but-inert (the #298/#302 precedent) or shape-preserved against a nonexistent target. v1alpha2-only, no v1alpha1 equivalent, no conversion-webhook impact. See `docs/tests/444/TEST_PLAN.md`.

**Addendum (v1.9, #443)**: `NetworkPolicyNamedIngressOverride.IngressNamespaces` under `Console` is intentionally left unwired -- no `consoleNetworkPolicy()` builder exists, and none is planned. Unlike Gateway/APIFrontend/DataStorage/KubernautAgent (whose ingress peers are a predictable, operator-known set of in-cluster components or the OCP ingress/monitoring namespaces), Console's ingress is a browser-facing UI endpoint whose legitimate callers are end-user-driven and deployment-specific, not something the operator can meaningfully default or override on the administrator's behalf. Documented in `docs/security/credentials-and-tls.md` (NetworkPolicy/SC-7 section) with guidance for administrators to supply their own supplemental `NetworkPolicy` scoped to `app: kubernaut-console`.
...
**Addendum (v1.7, #302)**: the shape-alignment above was never implemented for `InteractiveSpec.JWTProviders`/`AllowInsecureJWKS` specifically. Both fields are removed from `InteractiveSpec` entirely, in v1alpha1 and v1alpha2, rather than shape-aligned: KA's `interactive.jwtProviders[]` authenticator has no reachable caller in this operator's deployment. `kubernautAgentNetworkPolicy` (`internal/resources/networkpolicies.go`) admits only `AIAnalysis` and (when enabled) `APIFrontend` as ingress peers to KA's MCP port -- and per [kubernaut#1287](https://github.com/jordigilh/kubernaut/issues/1287), AF itself no longer forwards end-user JWTs to KA, instead authenticating with its own service-account bearer token (trusted-intermediary pattern). No client the operator's NetworkPolicy can ever admit would present a JWT to this endpoint, and the CRD exposes no field to add an additional ingress source. `JWTProviderSpec`/`ClaimMappingsSpec` themselves are unaffected -- `APIFrontendAuthSpec.JWTProviders` (F6's other consumer) still uses them for AF's own live multi-provider OIDC auth, which does have reachable callers (any client through AF's ingress).
